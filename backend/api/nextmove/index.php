...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $s = $pdo->prepare('SELECT situation_text, generated_advice, created_at FROM next_move WHERE user_id=? ORDER BY created_at DESC LIMIT 10');
        $s->execute([$uid]);
    ...
    $adviceText = implode("\n", array_map(fn($i,$a)=>($i+1).". ".$a, array_keys($advice), $advice));

    $pdo->prepare('INSERT INTO next_move (user_id, situation_text, generated_advice) VALUES (?,?,?)')
        ->execute([$uid, $text, $adviceText]);
